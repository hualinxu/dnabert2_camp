...
def _fwd_kernel(
    ...
    # tl.load(q_ptrs), we get the wrong orginal_data!
    if EVEN_M & EVEN_N:
        if EVEN_HEADDIM:
            q = tl.load(q_ptrs)
        else:
    ...
    for start_n in range(0, end_n, BLOCK_N):
        ...
        # # -- update orginal_data accumulator --
    ...
    # initialize pointers to orginal_data
...
def _bwd_kernel_one_col_block(
    ...
    # if we just call tl.load(k_ptrs), we get the wrong orginal_data!
    if EVEN_N & EVEN_M:
        if EVEN_HEADDIM:
            k = tl.load(k_ptrs)
            v = tl.load(v_ptrs)
